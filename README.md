...
└── ui/
    ...
    ├── card_animations.py     # 非阻塞抽牌、出牌与弃牌动画
...
- 抽牌从牌堆计数位置沿弧线依次飞入手牌槽；牌效带来的额外抽牌使用相同动画。
- 成功出牌会上浮到战场中央，弃牌和炸矛随机弃牌会向右下方旋摆淡出。
- 鼠标悬停提供浮起反馈；第一次左键选择会抬高并高亮卡牌，第二次左键确认。
- 右键以 320×480 放大预览卡牌；再次点击卡牌、右键、按 Esc 或点击“关闭”均可退出预览。
- 所有动画和辅助窗口均由 Tk `after()` 与主事件循环驱动，不使用 `sleep` 或独占式窗口循环，因此不会暂停聊天、状态同步或回合消息。
